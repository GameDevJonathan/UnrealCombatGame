...
bool UBlockComponent::Check(AActor* Opponent)
{
	...
	FVector OpponentFoward{ Opponent->GetActorForwardVector()};
	
	FVector PlayerForward{ CharacterRef->GetActorForwardVector() };

	double Result{ FVector::DotProduct(OpponentFoward, PlayerForward) };

	//dot product if negative : Facing each other / positve not facing
